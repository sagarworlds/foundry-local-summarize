...
- **Cited Document Chat**: Each co-pilot question is answered from the few document passages most relevant to it (BM25 retrieval), numbered so answers cite them as `[P#]`, plus the summary and the last few turns. The model is told to reply "The document does not say." rather than guess. Budgets live under `Foundry:Chat`.
